...
<e:switch value="${param.eaction}">
    ...
    <e:case value="villageCell_yx_summary">
        <e:q4l var="dataList">
            ...
            from(
            select count(distinct t.scene_id) scene_count,count(distinct t.order_id) yx_count,count(distinct t.order_id) order_count,
            count(decode(t.exec_stat,0,null,order_id)) zx_count,count(decode(t.SUCC_FLAG,1,order_id,null)) suc_count
    ...
    <e:case value="villageCell_yx_list">
        <e:q4l var="dataList">
            ...
            SELECT t.SERV_NAME,
            t.ACC_NBR,
            decode(t.CONTACT_TEL,'0',' ',t.CONTACT_TEL) USER_CONTACT_NBR,
            t.scene_id MKT_CAMPAIGN_ID,
            t.mkt_content MKT_CAMPAIGN_NAME,
            t.mkt_reason CONTACT_SCRIPT,
            C.ADDRESS_DESC,
            t.PROD_INST_ID,
            t.brigade_id,
            COUNT(1) OVER() c_num
            FROM
            <e:description>
            ${gis_user}.VIEW_GIS_ORDER_LIST_MON t,
            ${gis_user}.TB_GIS_USER_LIST_D B,
            </e:description>
            ${sql_part_tab1} t,
            ${gis_user}.TB_MKT_INFO C
            WHERE
            t.PROD_INST_ID = C.PROD_INST_ID
            ${sql_part_where1}
            AND t.VILLAGE_ID = '${param.village_id}'
            <e:if condition="${!empty param.build_id && param.build_id ne '-1'}">
                AND t.brigade_id = '${param.build_id}'
            </e:if>
    ...
    <e:case value="getSceneSelectOptionInDB">
        <e:q4l var="dataList">
            SELECT
            distinct t.scene_id MKT_CAMPAIGN_ID,
            t.mkt_content MKT_CAMPAIGN_NAME
            FROM
            <e:description>
            ${gis_user}.VIEW_GIS_ORDER_LIST_MON t,
