...
<e:switch value="${param.eaction}">
    ...
    <e:description>考核统计 日 已废弃，使用tabData_sandbox_summary_grid_yx中同名方法</e:description>
    ...
    <e:case value="get_yx_data">
        <e:q4o var="dataObject">
            ...
            where ACCT_DAY = TO_CHAR(sysdate,'yyyymmdd')
            and BRANCH_TYPE <> 'c1'
            and STAT_LVL = 4
            ...
            left join
            (select * from ${gis_user}.VIEW_GSCH_MKT_ORDER_STAT_DAY
            where ACCT_DAY = TO_CHAR(sysdate - 1,'yyyymmdd')
            ) T1
            on
            T.LATN_ID = T1.LATN_ID
            GROUP BY T.LATN_ID, T.LATN_NAME
        </e:q4o>${e:java2json(dataObject)}
    </e:case>
    ...
    <e:case value="get_info_all">
	    ...
	    '网格概述窗口' a,
            ...
            (SELECT COUNT(1) FROM ${gis_user}.view_db_cde_village a,${gis_user}.TB_GIS_RES_INFO_DAY b WHERE
            <e:if condition="${!empty param.grid_id_short }" var="empty_grid_id">
                a.grid_union_org_code = '${param.grid_id_short }'
            </e:if>
            <e:else condition="${empty_grid_id}">
                <e:if condition="${!empty param.grid_union_org_code}">
                    a.grid_id = (${sql_part_grid_id_short})
                </e:if>
            </e:else>
            AND a.village_id = b.latn_id AND DECODE(NVL(GZ_ZHU_HU_COUNT,0),0,0,NVL(GZ_H_USE_CNT, 0) / GZ_ZHU_HU_COUNT)<0.1) LOW_10_FILTER_VILLAGE_CNT,
    ...
    <e:description>网格汇总信息补充</e:description>
	...
	<e:case value="get_four_vill_type">
		<e:q4o var="dataObject">
			...
			FROM ${gis_user}.tb_gis_village_edit_info where grid_id_2 =
            <e:if condition="${!empty param.grid_id_short }" var="empty_grid_id">
                '${param.grid_id_short }'
            </e:if>
            <e:else condition="${empty_grid_id}">
	...
	<e:case value="get_buss_cnt">
		<e:q4o var="dataObject">
            ...
            <e:else condition="${empty_grid_id}">
                <e:if condition="${!empty param.grid_union_org_code}">
                    and a.grid_union_org_code = ${param.grid_union_org_code}
                </e:if>
            </e:else>
            </e:description>
            select
            ...
            <e:else condition="${empty_grid_id}">
                <e:if condition="${!empty param.grid_union_org_code}">
                    and latn_id = (select distinct grid_id from ${gis_user}.db_cde_grid where grid_union_org_code = '${param.grid_union_org_code}')
                </e:if>
            </e:else>
		</e:q4o>${e:java2json(dataObject)}
	</e:case>
	...
	<e:case value="get_add4_data">
		<e:q4o var="dataObject">
			select count(1) cnt from SDE.TB_GIS_MAP_SEGM_LATN_MON where
            <e:if condition="${!empty param.grid_id_short }" var="empty_grid_id">
                grid_id = '${param.grid_id_short }'
            </e:if>
            <e:else condition="${empty_grid_id}">
                <e:if condition="${!empty param.grid_union_org_code}">
                    grid_union_org_code = ${param.grid_union_org_code}
                </e:if>
            </e:else>
		</e:q4o>${e:java2json(dataObject)}
	</e:case>
