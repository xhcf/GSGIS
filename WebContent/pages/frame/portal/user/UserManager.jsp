...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function formatterCZ(value,rowData){
				var content ='<a href="javascript:void(0);" style="text-decoration: none;margin: 0 5px;" onclick="toEdit(\''+rowData.USER_ID+'\')"><i class="fa fa-edit"></i></a>'+
				             '<a href="javascript:void(0);" style="text-decoration: none;margin: 0 5px;" onclick="doDelete(\''+rowData.USER_ID+'\')"><i class="fa fa-trash"></i></a>';
				    content +='<a href="javascript:void(0);" style="text-decoration: none;margin: 0 5px;" onclick="toAuthor(\''+rowData.USER_ID+'\')"><i class="fa fa-lock" aria-hidden="true"></i></a>';
				    content +='<a href="javascript:void(0);" style="text-decoration: none;margin: 0 5px;" onclick="toUserRoles(\''+rowData.USER_ID+'\')"><i class="fa fa-user" aria-hidden="true"></i></a>';
				return content;
	...
	<div id="tbar" class="newSearchA">
			...
			<div class="search-area">
				...
				性别：<e:select id="user_sex" name="user_sex" items="${SexList.list}" style="min-width:100px" label="TYPE_DESC" value="TYPE_CODE" headLabel="全部" headValue=""  defaultValue="${param.user_sex}"/>
				<!-- 如果用户是管理员，才可见，否则 不能更改该属性 -->
				管理员：<e:select id="user_admin" style="min-width:100px" name="user_admin" items="${AdminTypeList.list}" label="TYPE_DESC" value="TYPE_CODE" headLabel="全部" headValue=""  defaultValue="${param.user_admin}"/>
				用户状态：<e:select id="user_state" style="min-width:100px" name="user_state" items="${StateList.list}" label="TYPE_DESC" value="TYPE_CODE" headLabel="全部" headValue=""  defaultValue="${param.user_state}"/>
		...
		<c:datagrid url="/pages/frame/portal/user/UserAction.jsp?eaction=list&loginid=${param.loginid}&area_no=${param.area_no }&username=${param.username}&user_sex=${param.user_sex}&user_admin=${param.user_admin}&user_state=${param.user_state}" id="userTable" pageSize="15" fit="true"
					download="true" nowrap="false" border="false" style="width:auto;" toolbar="#tbar">
			<thead>
				<tr>
					...
					<th field="REG_DATE" width="90" align="center">
						注册时间
					</th>
					<th field="UPDATE_DATE" width="90" align="center">
						更新时间
					</th>
