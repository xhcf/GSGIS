...
$(function(){
    ...
    openNewWinInfoCollectEdit = function (segment_id,tab_id) {
        if(tab_id==undefined)
            tab_id = 0;
        $.post(appBase+"/pages/telecom_Index/common/sql/viewPlane_custom_action.jsp",{"eaction":"prod_list","segment_id":segment_id},function(data){
            var d = $.parseJSON(data);
            if(d.length>1){
                $("#prod_list_win > iframe").attr("src", appBase+"/pages/telecom_Index/sub_grid/viewPlane_prod_list.jsp?segment_id=" + segment_id + "&city_id=" + city_id);
