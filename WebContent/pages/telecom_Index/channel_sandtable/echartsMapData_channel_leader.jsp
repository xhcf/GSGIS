...
<e:switch value="${param.eaction}">
  ...
  <e:case value="index_dispatch_province">
    <c:tablequery>
        SELECT a.latn_Id,a.latn_name area_desc,
        b.order_cnt,b.exec_cnt,b.ord,use_rate,use_rate1,succ_lv
        FROM
        (
        SELECT DISTINCT latn_id,latn_name FROM ${gis_user}.db_cde_grid
        UNION ALL
        SELECT 999 latn_id,'全省' latn_name FROM dual
        )a
        LEFT JOIN
        (SELECT *
        ...
        WHERE 1 = 1
        AND STAT_DATE = TO_CHAR(SYSDATE, 'YYYYMMDD')
        AND STAT_LVL = '1'
        AND BRANCH_TYPE <> 'c1'
        UNION
        SELECT LATN_ID || '' latn_id,
        LATN_NAME AREA_DESC,
        LATN_SORT ord,
        SUM(COL_NUM_1) ORDER_CNT,
        SUM(COL_NUM_2) EXEC_CNT,
        ...
        AND STAT_LVL = '1'
        AND BRANCH_TYPE <> 'c1'
        GROUP BY LATN_ID, LATN_NAME, LATN_SORT)
        where 1=1
        <e:if var="haveQueryList"  condition="${!empty param.condition_str}">
            and (${param.condition_str})
        </e:if>)b
  ...
  <e:case value="index_bureau1">
    <c:tablequery>
      select * from(
      SELECT DISTINCT
      a.latn_id,b.latn_name,b.bureau_name AREA_DESC,
      CASE
  ...
  <e:case value="index_bureau">
    <c:tablequery>
       SELECT '地市地图悬浮窗',
    <e:if var="isProvince"  condition="${!empty param.flag && param.flag eq '1'}">
        T.CHANNEL_TYPE_NAME_QD || '_' ||T.AREA_DESCRIPTION NAME,
    </e:if>
    <e:else condition="${isProvince}">
   		T.LATN_ID,
   		T.CHANNEL_TYPE_NAME_QD NAME,
    </e:else>
      	T.CHANNEL_NUM VALUE,
        T.AVG_SCORE,
        T.QDJF_CUR_MONTH CHANNEL_SCORE_SUM,
        T.AVG_CHANNEL_QDJF AVG_CHANNEL_SCORE,
        c.zero_sale_channel,
        c.low_sale_channel,
        c.high_channel
 	FROM ${channel_user}.TB_QDSP_TYPE_CHANNEL_M T,
 	${channel_user}.TB_QDSP_STAT_VIEW_M c
 	WHERE 1 = 1
	 	AND T.FLAG = '${param.flag}'
	 	AND T.ACCT_MONTH='${param.acct_month}'
	 	AND T.CHANNEL_TYPE_NAME_QD <> '合计'
	 	and c.flag = '${param.flag}'
	 	and t.latn_id = c.latn_id
	 	and c.acct_month = '${param.acct_month1}'
    </c:tablequery>
  </e:case>

<e:description>效能概览-门店类型统计</e:description>
    <e:case value="index_channel_type">
        <e:q4l var="index_channel_type">
          SELECT * FROM(
		  ...
		  <e:if condition="${not empty param.region_id && !(param.region_id eq null)}">
            ...
            GROUP BY t.channel_type_name_qd,
			          ...
			          T.QDJF_CUR_MONTH,
			          T.AVG_CHANNEL_QDJF
        </e:q4l>${e: java2json(index_channel_type.list) }
    </e:case>
  ...
  <e:case value="index_dispatch_bureau">
    <c:tablequery>
        ...
        FROM
        (SELECT DISTINCT latn_id,latn_name,bureau_no,bureau_name,c.region_order_num FROM ${gis_user}.db_cde_grid c
        WHERE
        BRANCH_TYPE <> 'c1'
        <e:if condition="${param.flag eq '2'}">
            <e:if condition="${!empty param.region_id}">
        ...
        GROUP BY latn_id, latn_name,bureau_no, bureau_name, bureau_sort)b
        on a.bureau_no = b.bureau_no
        where 1=1
        <e:if var="haveQueryList"  condition="${!empty param.condition_str}">
            and (${param.condition_str})
        </e:if>
        ORDER BY ord,region_order_num
    </c:tablequery>
  </e:case>
  ...
  <e:case value="index_sub">
    <c:tablequery>
       SELECT '区县地图 悬浮窗口',
    <e:if var="isSub"  condition="${!empty param.flag && param.flag eq '1'}">
        T.CHANNEL_TYPE_NAME_QD || '_' ||T.AREA_DESCRIPTION NAME,
   </e:if>
   <e:else condition="${isSub}">
   		T.LATN_ID,
   		T.CHANNEL_TYPE_NAME_QD || '_' ||
   		  DECODE(b.BUREAU_NAME,'河西堡营销中心（县）','城市营销中心','西城营销中心','城市销售中心',T.BUREAU_NAME)  NAME,
   </e:else>
      	T.CHANNEL_NUM VALUE,
        T.AVG_SCORE,
        T.QDJF_CUR_MONTH CHANNEL_SCORE_SUM,
        ...
        c.low_sale_channel,
        c.high_channel
 	FROM ${channel_user}.TB_QDSP_TYPE_CHANNEL_M T,${channel_user}.tb_gis_qd_bureau_ref b,${channel_user}.TB_QDSP_STAT_VIEW_M c
 	WHERE 1 = 1
	 	AND T.FLAG = '${param.flag}'
	 	AND T.LATN_ID = '${param.region_id}'
	 	AND T.ACCT_MONTH='${param.acct_month}'
	 	...
	 	AND T.CHANNEL_TYPE_NAME_QD <> '合计'
	 	and t.bureau_no = b.bureau_no
	 	and c.flag = '${param.flag}'
	 	and t.bureau_no = c.bureau_no
	 	and c.acct_month = '${param.acct_month1}'
    </c:tablequery>
  </e:case>
  ...
  <e:case value="index_collect_sub">
   ...
   <e:case value="index_dispatch_sub">
    <c:tablequery>
        select * from(
        SELECT latn_name,branch_union_org_code || '' UNION_ORG_CODE,
        ...
        GROUP BY latn_name,branch_union_org_code, branch_name, branch_sort)
        where 1=1
        <e:if var="haveQueryList"  condition="${!empty param.condition_str}">
            and (${param.condition_str})
        </e:if>
  ...
  <e:case value="index_grid">
    <c:tablequery>
      SELECT *
      FROM (SELECT DISTINCT A.LATN_ID,b.latn_name,
      ...
      <e:if condition="${param.flag eq '3'}">
        <e:if condition="${!empty param.region_id}">
          and bureau_name = '${param.region_id}'and latn_id = '${param.city_id}'
        </e:if>
      </e:if>
      <e:if condition="${!empty param.name}">
...
<e:case value="index_dispatch_grid">
    <c:tablequery>
        select * from (
        SELECT grid_union_org_code || '' latn_id,latn_name,
        grid_name AREA_DESC,
        grid_sort || '' ord,
        SUM(COL_NUM_1) ORDER_CNT,
        SUM(COL_NUM_2) EXEC_CNT,
        ...
        GROUP BY latn_name,grid_union_org_code, grid_name, grid_sort)
        where 1=1
        <e:if var="haveQueryList"  condition="${!empty param.condition_str}">
            and (${param.condition_str})
        </e:if>
  ...
  <e:case value="index_village">
    ...
    <e:case value="index_village_count_limited">
        <c:tablequery>
            select * from(
            SELECT *
            FROM (SELECT DISTINCT A.LATN_ID,latn_Name,
            B.village_name AREA_DESC,
            CASE
            WHEN NVL(GZ_ZHU_HU_COUNT, 0) = 0 THEN
            '0.00%'
   ...
   <e:case value="index_dispatch_village">
    <c:tablequery>
        ...
        c.branch_no in (
         select distinct union_org_code from ${gis_user}.db_cde_grid bb
            ...
            <e:if condition="${param.flag eq '3'}">
                <e:if condition="${!empty param.region_id}">
                    and bb.bureau_name = '${param.region_id}'
    ...
    <e:case value="index_dispatch_village_limited">
        <c:tablequery>
            ...
            <e:if condition="${param.flag eq '3'}">
                <e:if condition="${!empty param.region_id}">
                    and c.bureau_name = '${param.region_id}'
    ...
    <e:case value="index_dispatch_village_cnt">
        <e:q4o var="dataObject">
            ...
            DECODE(USER_NUM, 0, 0, to_char(ROUND(SUCC_NUM / USER_NUM, 4) * 100,'FM999999990.00')) || '%' SUCC_LV,
            nvl(DECODE(USER_NUM, 0, 0, ROUND(SUCC_NUM / USER_NUM, 4) * 100),0) SUCC_LV1,
            1 ORDER_NUM
            FROM (SELECT BB.VILLAGE_ID,
            COUNT(AA.ORDER_ID) ORDER_NUM,
            COUNT(DECODE(EXEC_STAT, 0, AA.ORDER_ID)) EXEC_NUM,
  ...
  <e:case value="grid_list_for_village_cell">
    <e:q4l var="dataList">
        ...
        B.ZOOM,
        B.GRID_ZOOM
        FROM edw.vw_tb_cde_village@gsedw A,${gis_user}.DB_CDE_GRID B,
