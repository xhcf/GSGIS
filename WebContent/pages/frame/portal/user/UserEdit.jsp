...
<html>
	<head>
		...
		function doEdit(){
			if($('#userForm').form('validate')){
				...
				var area_name = $("#dim_AREA_NO").combobox("getText");
				/*var area_full_name = "";
				if(city_name_speical.indexOf(area_name)>-1)
					area_full_name = area_name + "州";
				else
					area_full_name = area_name + "市";*/
			 	$("#dim_AREA_NAME").val(area_name);
				var bureau_name = $("#dim_CITY_NO").combobox("getText");
				$("#dim_CITY_NAME").val(bureau_name.substr(bureau_name.indexOf(".")+1));
				$("#dim_SUB_NAME").val($("#dim_TOWN_NO").combobox("getText"));
				$("#dim_GRID_NAME").val($("#dim_GRID_NO").combobox("getText"));
	...
	<body>
		<form id="userForm" method="post" action="#" style="height:100%;">
			...
			<div class="easyui-tabs mTop15" fit="true" plain="true" border="false" style="height:360px;">
				<div title="基本信息">
					...
					<table class="pageTable">
						...
						<e:forEach items="${attr_list.list}" var="item">
							<e:switch value="${item.SHOW_MODE}">
								<e:case value="INPUT">
									<e:if var="need_hide" condition="${item.ATTR_CODE eq 'CITY_NAME' || item.ATTR_CODE eq 'SUB_NAME' || item.ATTR_CODE eq 'AREA_NAME' || item.ATTR_CODE eq 'GRID_NAME'}">
										<tr style="display:none;">
									</e:if>
									<e:else condition="${need_hide}">
										<tr>
									</e:else>
