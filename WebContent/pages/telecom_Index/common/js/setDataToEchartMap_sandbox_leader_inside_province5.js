...
function setDataToEchartMap5(params,url4realTimeSubList,url4mapToWhere){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(d.length==0)
                ...
                for (var i = 0; i < d.length; i++) {
                    ...
                    datas.push(obj);
                    datas2[org_name_temp] = [d[i].USE_RATE,d[i].SHOULD_COLLECT_CNT,d[i].ALREADY_COLLECT_CNT,d[i].OTHER_MON_RATE,d[i].OTHER_YEAR_RATE];
                    //[d[i].USE_RATE,d[i].SHOULD_COLLECT_CNT,d[i].ALREADY_COLLECT_CNT,d[i].OTHER_MON_RATE,d[i].OTHER_YEAR_RATE];
                }
        ...
        complete:function(){
            echartMapReset5(min,max,params,series_name,color,datas,datas2);
            chart_map.hideLoading();
        }
    });
}
//地市 区域块
function echartMapReset5(min_num,max_num,params,series_name,color,data,data2){
