...
function setDataToEchartMap(params,url4echartmap,url4mapToWhere,url4devTabToWhere){
    ...
    $.ajax({
        ...
        complete:function(){
            echartMapReset(min,max,params,series_name,color,datas1,datas2,datas3,datas4);
            //chartClickAction(url4echartmap,url4mapToWhere,url4devTabToWhere);
            chart.hideLoading();
        }
    });
...
function echartMapReset(min_num,max_num,params,series_name,color,data1,data2,data3,data4){
    ...
    var option = {
        ...
        tooltip: {
            ...
            formatter: function(params) {
                ...
                //var myseries = ["住户数","竞争住户数","收集占比","预警住户"];
                var myseries = option.series;
                for (var i = 0; i < myseries.length; i++) {
                    for(var j=0;j<myseries[i].data.length;j++){
                        if(myseries[i].data[j].name==params.name){
                            if(myseries[i].name=='收集占比')
                                res+="<tr><td>"+myseries[i].name +":</td><td>"+myseries[i].data[j].value+"%</td></tr>";
                            else if(myseries[i].name=='竞争住户' || myseries[i].name=='预警住户')
                                res+="<tr><td>"+myseries[i].name +":</td><td>"+myseries[i].data[j].value+"</td></tr>";
                        }
                    }
                }
