...
<e:q4o var="org_level3">
	<e:if condition="${sessionScope.UserInfo.LEVEL eq '3' || sessionScope.UserInfo.LEVEL eq '4' || sessionScope.UserInfo.LEVEL eq '5' }" var="level3">
		select distinct bureau_no,bureau_name from ${channel_user}.tb_gis_qd_bureau_ref where bureau_no_hx = '${sessionScope.UserInfo.CITY_NO}'
	</e:if>
	<e:else condition="${level3}">
		select '' bureau_no,'' bureau_name from dual	
	</e:else>
</e:q4o>
...
<head>
  ...
  <link href='<e:url value="/pages/telecom_Index/channel_sandtable/css/leader_org_frame.css?version=1.5"/>' rel="stylesheet" type="text/css" media="all" />
  <link href='<e:url value="pages/telecom_Index/common/css/layer_win.css?version=1.6" />' />
    <link href='<e:url value="/pages/telecom_Index/channel_sandtable/css/sp_channel.css"/>' rel="stylesheet" type="text/css" media="all" />
    <style>
    .entrance_btn_group{
      position: absolute;
      left:10px;
...
<div class="g_wrap clearfix main easyui-layout" id="map_mode_view" style="position:absolute;top:0;left:0;">
  <div data-options="region:'west'" style="height:100%;" class="panes" id="include_map">
    <div id="tab_content" style="height:99.6%;padding-top:5px;width:99.8%; margin:0 auto">
      <div class="new_map c_frame_box" style="width: 100%">
        <div id="mainleft" class="mainleft c_main_fl">
          <h2 id="click_path"></h2>
        ...
        <div id="frmTitle" class="frmTitle c_main_fr" name="fmTitle">
...
<script type="text/javascript">
  ...
  var user_bureau_no = '${org_level3.BUREAU_NO}';//'${sessionScope.UserInfo.CITY_NO}';//用户的分局编码
  ...
  global_position_url[0] = '<e:url value="/pages/telecom_Index/channel_sandtable/leader_province_level.jsp" />';

  var url4Query = '<e:url value="/pages/telecom_Index/common/sql/tabData.jsp"/>';

  var change_echarts_range = "";

  var map_url_province = '<e:url value="/pages/telecom_Index/channel_sandtable/leader_province_level.jsp" />';
  var map_url_city = '<e:url value="/pages/telecom_Index/channel_sandtable/leader_city_level.jsp" />';
  var map_url_bureau = '<e:url value="/pages/telecom_Index/channel_sandtable/leader_bureau_level.jsp" />';

  var map_url_sch_ent = '<e:url value="/pages/telecom_Index/channel_sandtable/enterprise_bureau_level.jsp" />';
  ...
  if(user_level=="" || user_level==undefined){
    ...
    //市级用户
    else if(user_level==2){///渠道积分强制市级用户 恢复
    //else if(user_level==2 || user_level==3 || user_level==4 || user_level==5){///渠道积分强制市级用户
      global_parent_area_name = province_name;
      ...
      //global_current_index_type = default_show_index;//右侧曲线图默认显示的标签卡，此处配置是“移动”
      global_current_flag = 2;///user_level;//这里将按权限改动： 根据用户所在级别定义，省为1，市2，区县3，支局4，网格5，此处配置1。查询时加1
  ...
  var force_city_level = false;//强制跳转到地市地图
  if(!force_city_level){
  	if(user_level>=3){
  		if(user_bureau_no==null || user_bureau_no=="" || user_bureau_no==undefined){
  			layer.msg("该用户没有对应的渠道组织机构，请联系管理员");
  		}else{
		    ...
		    global_current_full_area_name = '${org_level3.BUREAU_NAME}';///'${sessionScope.UserInfo.CITY_NAME}';
		
		    global_position[2] = '${org_level3.BUREAU_NAME}';///'${sessionScope.UserInfo.CITY_NAME}';
		    global_position[1] = global_parent_area_name;
		
				global_current_area_name = '${org_level3.BUREAU_NAME}'///'${sessionScope.UserInfo.CITY_NAME}';
				
				//强制跳转到区县一层
		    global_current_flag = 3; ///user_level;//这里将按权限改动： 根据用户所在级别定义，省为1，市2，区县3，支局4，网格5，此处配置1。查询时加1
		    global_current_city_id = '${sessionScope.UserInfo.AREA_NO}';
		
		    //if(user_level==3)
		      mapContainer_url = map_url_bureau+'?city_id='+user_city_id+'&bureau_no='+user_bureau_no;
  ...
  var st_broad_home = '<e:url value="/pages/telecom_Index/channel_sandtable/smallTab/small_table_broad_home_market.jsp" />';
  var st_fb_cover = '<e:url value="/pages/telecom_Index/channel_sandtable/smallTab/small_table_fb_cover.jsp" />';
  var st_collect = '<e:url value="/pages/telecom_Index/channel_sandtable/smallTab/small_table_collect.jsp" />';
  var st_dispatch_yx = '<e:url value="/pages/telecom_Index/channel_sandtable/smallTab/small_table_dispatch_yx.jsp" />';
  var st_fb_real_percent = '<e:url value="/pages/telecom_Index/channel_sandtable/smallTab/small_table_fb_real_percent.jsp" />';
  var st_protection = '<e:url value="/pages/telecom_Index/channel_sandtable/smallTab/small_table_protection.jsp" />';
  ...
  $(function() {
    ...
    if('${param.global_add4_position_flag}'==1){
      debugger;
      load_list_tab_add4();
    }
  ...
  function inside_channel_model(region_id,region_name,region_type){
    if(region_type=='1'){//地市
      switchToCity(region_id,region_name);
    }else if(region_type=='2'){//区县
      switchToBureau(region_id,region_name);
    }else if(region_type=='3'){//渠道
      switchTochannel(region_id,region_name);
    }
  }
  ...
  function toCity(){
    ...
    if(user_level<3)
      ...
      url = map_url_bureau+'?city_id='+user_city_id+'&bureau_no='+user_bureau_no;
  ...
  function load_list_view(index_type,flag,region_id,from_menu){
    ...
    var tab_url = '<e:url value="/pages/telecom_Index/channel_sandtable/bigTab/"/>';
    if(index_type==2.2)
    	tab_url += "big_table_channel_sale_analyse_org_framework.jsp";
    else
  ...
  function load_list_view2(city_id,village_type,beginDate){
      $("#click_path").hide();
    //2018.10.19 校园政企新模块
      $("#list_mode_view").empty();
    /*$("#list_mode_view").slideDown(function(){
    ...
    "&city_id="+city_id+"&village_type="+village_type + "&beginDate=" + beginDate;
  ...
  function load_list_tab_add4(){
    ...
    $("#list_mode_view").load('<e:url value="/pages/telecom_Index/tab_add4/tab_add4_tab.jsp"/>?city_id='+city_id+'&bureau_id='+bureau_id);
    //$("#list_mode_view").load('<e:url value="/pages/telecom_Index/tab_add4/tab_add4_org_framework.jsp"/>');
  }
  ...
  function switchToBureau(region_id,bureauName){
    global_current_full_area_name = bureauName;
    global_current_area_name = bureauName;
    global_current_flag = 3;

    global_position.splice(2,1,bureauName);
  ...
  function select_option(entrance_type,bureau_name,bureau_no,branch_name, union_org_code, index, city_id, city_name, grid_name,station_id,grid_union_org_code,village_id) {
    ...
    if(entrance_type=="sub"){
      ...
      global_position_param[4] = station_id;
      global_current_flag = 5;
    }else if(entrance_type=="village_cell"){
      global_entrance_region_id = station_id;
      global_report_to_id = station_id;
      global_grid_id = grid_union_org_code;
      global_position[4] = grid_name;
