...
var ${id} = {
	...
	},reloadMyKpi:function(){
		kpiSelectorService.getMyKpiData(${id}.cubeId,StoreData.xid,function(data,exception){
				${id}.generateHtml("mykpi",data["mykpi"]);
		});
	},
	...
	removeFavorite:function(itemId){//删除收藏
		kpiSelectorService.removeFavorite(${id}.cubeId,itemId,function(data,exception){
			if(data=="0"){
				$.messager.alert("系统提示","删除收藏失败!","info");
			}else{
	...
	getItemObj:function($li,tabType){//根据编号获得当前操作的指标、维度或属性
			...
			if(${id}.currentItem.category=="extcolumn"){
				${id}.currentItem.id="extcolumn_"+$li.attr("key");
				${id}.currentItem.column="extcolumn_"+$li.attr("column");
				${id}.currentItem.kpiType='extcolumn';
				${id}.currentItem.isextcolumn='true';
				${id}.currentItem.extcolumnid=$li.attr("key");
				${id}.currentItem.isextcolumn='true';
			}
	...
	open:function(cubeId){//打开组件
		...
		$("div[id^='caculateDialog_']").dialog('close');//关闭指标库计算列
		${id}.cubeId=cubeId;
		...
		if(category=='kpi'){//生成指标html
			$.each(group,function(groupIndex,groupItem){
				...
				$.each(children,function(childIndex,childItem){
					...
					gParamMap[childItem.id] = childItem.desc;
					${id}.kpiDataTypeMap[childItem.id] = kpiType;
					...
					if(childItem.children!=null&&childItem.children.length>0){
						...
						for(var i=0;i<childItem.children.length;i++){
							gParamMap[childItem.children[i].id] = childItem.children[i].desc;
						}
					}
	...
	// 				html = "<ul>";
			...
			$.each(extcolList,function(extIndex,extItem){
				availableTags.push({value:extItem.text,data:'1'});
				html+="<li class='default' category='extcolumn' key='"+extItem.id+"' column='"+extItem.id+"' node-id='"+extItem.id+"' draggable='true' desc='"+extItem.text+"'><b>"+extItem.text+"</b><a href='javascript:void(0);' class='edit_col_btn'></a> <a href='javascript:void(0);' class='starLink_close'>×</a></li>\n";
			});
			html+="</ul></div>";
			...
			$("#extcolList_${id}>ul>li>a").click(function(event){//计算列按钮点击事件
				event.stopPropagation();
			    if($(this).attr("class")=="starLink_close"){//删除计算列
			    	var extcolId = $(this).parent().attr("key");
			    	var extcolName = $(this).parent().attr("desc");
			    	removeCaculateCol(extcolId,extcolName);
			    }else{
			    	var extcolId = $(this).parent().attr("key");
			    	editCaculateCol(extcolId);
			    }
			});

			$("#mykpiKeywords_${id}").autocomplete({
		         lookup: availableTags,
		         onSelect: function (suggestion) {}
		    }); 
		}
...
$(function(){
	...
	$("#closeDialogBtn_${id}").click(function(){
		$("#container_${id}").hide();
		$("div[id^='caculateDialog_']").dialog('close');
...
//刷新我的指标tab页
function reloadMyKpiTab(){
	${id}.reloadMyKpi();
}

function getCurrentDragItemType(){
	return ${id}.getCurCategory();
}
</script>
<div id="container_${id}" class="kpiSelectorCon" kpiSelectorDiv="true">
	...
	<div id="tabs_${id}" class="easyui-tabs easyui-tabs-kpiTag" data-options="tools:'#tab-tools_${id}'" style="width:320px;background:#fff; ">
		    <div title="指标">
			...
			<div title="我的指标">
				<div id="mykpiTool_${id}" class="pt40">
				    <div class="kpiSearchArea">
					    	...
					    	<a class="inputBtn" href="javascript:void(0)" class="easyui-linkbutton" onclick="queryBtnClick_${id}()">查询</a>
					    	<a class="foldBtn addBtn" href="javascript:void(0)" id="openCaculateDialog_${id}" onclick="openCaculateDialog()" state="closed" title="添加指标"><i class="fa fa-plus-square-o" aria-hidden="true"></i></a>
					  </div>
