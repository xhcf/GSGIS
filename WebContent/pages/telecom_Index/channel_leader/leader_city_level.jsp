...
<head>
  ...
  <script src='<e:url value="/pages/telecom_Index/channel_leader/js/channel_geo_center.js?version=New Date()"/>' charset="utf-8"></script>
  <script src='<e:url value="/pages/telecom_Index/common/js/ieBrowser.js"/>' charset="utf-8"></script>
  <script src='<e:url value="/pages/telecom_Index/channel_leader/js/xnglToEchartMap_channel_leader_inside_province.js?version=New Date()"/>'
...
<div style="position:relative;">
  <div class="leader_tools" id="tools_scroll">
    <ul class="nav_index_group">
      <li id="nav_index_broad_home_percent"><span></span><a href="javascript:void(0)" id="">效能概览</a></li>
...
<script type="text/javascript">
  ...
  var sql_url = '<e:url value="/pages/telecom_Index/channel_leader/channel_action/channel_leaderAction.jsp" />?eaction=getIndexForEcharts&flg=2&acc_day='+${initMonth};
  var sql_url1 = '<e:url value="/pages/telecom_Index/channel_leader/channel_action/channel_leaderAction.jsp" />?eaction=xn_rank&region_type=2&acct_month='+${initMonth}+"&region_id="+city_id;
  var url4echartmap = "";
  //echart地图要加载的页面
  var url4mapToWhere = '<e:url value="/pages/telecom_Index/channel_leader/leader_city_level.jsp"/>';
  ...
  var url4mapInsideWhere = '<e:url value="/pages/telecom_Index/channel_leader/leader_bureau_level.jsp"/>';
    ...
    $(".nav_index_group").children().each(function(){
      $(this).on("click",function(){
        ...
        if(index_type_temp==1){
          parent.load_list_view(1,flag,city_id);
          //parent.load_list_view(1,'','');
        }
        //else if(index_type_temp==7){
          //2018.10.19
        //  parent.switchToCity_inside_page(city_id,city_full_name,0,"sch_ent");
        //}
        else{
          if(index_name=="nav_index_view_video"){
            parent.view_video();
  ...
  function switch_region(region_type){
    ...
    if(region_type=="city"){
      ...
      url4devTabToWhere = '<e:url value="/pages/telecom_Index/channel_leader/smallTab/small_table_broad_home_market_city.jsp" />';
      load_map("bureau");
      load_index(2,index_type,city_id,region_type);
    }else if(region_type=="sub"){
      $(".region_switch_group li:eq(2)").addClass("active");
      url4devTabToWhere = '<e:url value="/pages/telecom_Index/channel_leader/smallTab/small_table_broad_home_market_sub.jsp" />';
      load_map("sub");
      load_index(3,index_type,city_id,region_type);
    }else if(region_type=="grid"){
      $(".region_switch_group li:eq(3)").addClass("active");
      url4devTabToWhere = '<e:url value="/pages/telecom_Index/channel_leader/smallTab/small_table_broad_home_market_grid.jsp" />';
      load_map("grid");
      load_index(4,index_type,city_id,region_type);
    }else if(region_type=="village"){
      $(".region_switch_group li:eq(4)").addClass("active");
      url4devTabToWhere = '<e:url value="/pages/telecom_Index/channel_leader/smallTab/small_table_broad_home_market_village.jsp" />';
      load_map("village");
  ...
  function load_map(region_type){
    ...
    if(region_type=="bureau"){
      ...
      url4mapToJson = '<e:url value="/resources/component/echarts_new/geoJson/gansu/" />';
      var city_name_full = "";
      ...
      url4mapToJson += map_json+".json?version=1";
      $.get(url4mapToJson, function (cityJson) {
        ...
        if(index_type== 0){
            ...
            setDataToEchartMap_bar_sub(params, url4echartmap, url4mapToWhere,sql_url+'&region_id='+city_id,sql_url1);
