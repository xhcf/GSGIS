...
<%--<script src='<e:url value="/pages/telecom_Index/js/freshTab.js"/>' charset="utf-8"></script>
                ...
                if(index==1){
					...
					$("#sub4").datagrid({
						...
						columns:[[
							...
							{field:'MARKET_LV',title:'市场渗透率',align:'center',halign:'center',width:sub_width*0.15,
								formatter:function(value,rowData){
									if(value==-1)
										return "";
									return value;
							...
							{field:'H_USER_CNT',title:'光宽用户',align:'center',halign:'center',width:sub_width*0.15,
								formatter:function(value,rowData){
									if(value==-1)
										return "";
									return value;
							...
							{field:'BUILD_CNT',title:'楼宇数',align:'center',halign:'center',width:sub_width*0.15,
								formatter:function(value,rowData){
									return value;
								}
							}
						...
						onLoadSuccess:function (data) {
							...
							if (data.rows.length == 0) {
								...
								var rows = $("#sub4").datagrid("getRows");
								...
								for (var j = 0; j < copyRows.length; j++) {
									//删除行
									var index = $('#sub4').datagrid('getRowIndex', copyRows[j]);
									$('#sub4').datagrid('deleteRow', index);
								}
  ...
  <script>
	...
	var url4Query_index = '<e:url value="pages/telecom_Index/common/sql/tabData_index_new.jsp" />';
	...
	$("#village").on("click",function(){
		...
		$("#village_detail_list").datagrid({
			url:url4Query_index,
			queryParams:{
				eaction:'getVillageMarketBySubstation',
				substation:substation
			},
			fitColumns:false,
			columns:[
				[
					...
					{field:'VILLAGE_NAME',title:'小区名称',align:'left',halign:'center',width:sub_width*0.32,
						formatter:function(value,rowData){
							...
							if(width>=1520)length_standard=8;
							if(value.length>length_standard)
								return "<span title='"+value+"' style=\"color:white;text-decoration: underline;cursor:pointer;\" onclick=\"toMap_village('"+rowData.VILLAGE_ID+"')\">"+value.substr(0,length_standard)+"..</span>";
							else{
								if(value=='合计')
   ...
   /* function show1(obj,id) {
    ...
    $(function(){
		...
		/*$.post(url4Query, {eaction: 'getDayNew', date: '${yesterday.VAL}', substation: substation}, function (data) {
			data = $.parseJSON(data)
			$("#m_n").text(data.M_N)
			$("#b_n").text(data.B_N)
			$("#i_n").text(data.I_N)
		...
		//20190109
        /*if(isIE()) {
            $("#sub").height($("#mark_sub").height()*0.9);
        }
        $("#sub").height($("#mark_sub").height()*0.9);
        ...
        /*$("#sub").datagrid({
            url:url4Query,
            queryParams:{
                /!*eaction:'getSubInfoBySubstation_zyl',
                yesterday:'${yesterday.VAL}',
                last_month:'${lastMonth.VAL}',*!/
                eaction:"getSubInfoBySubstation_zyl",
                substation:substation,
            ...
            columns:[
                [
                {field:'AREA',title:'地域',align:'center',halign:'center',width: sub_width*0.34,
                    formatter:function(value,rowData){
                    	//return value;
                       return '<div class="point">'+revetNameByRn(rowData.RN)+'</div>'+value;
										}
                },
                {field:'ZYL',title:'市场渗透率',align:'center',halign:'center',width:sub_width*0.17,
                    formatter:function(value,rowData){
                    	if(value!='--'){
                    		return "<span style='color: #fa8513'>"+value+"</span>";
                    	}else{
                      ...
                      /!*if(isNaN(parseFloat(value)))
                            return"";
                        if(value!=null&&value!=undefined&&value!=-1)
                            return "<span style='color: #fa8513'>"+value.toFixed(1)+"</span>"+"<span style='color: #fa8513'>"+"%"+"</span>";
                        else
                            return "--";*!/
                    }
                },/!*
                {field:'CDMA_NUM_REATE',title:'移动<br/>渗透率',align:'center',halign:'center',width:sub_width*0.14,
                    formatter:function(value,rowData){
                        if(isNaN(parseFloat(value)))
                            return"";
                        if(value!=null&&value!=undefined&&value!=-1)
                ...
                {field:'BRD_NUM_RATE',title:'宽带<br/>渗透率',align:'center',halign:'center',width:sub_width*0.14,
                    formatter:function(value,rowData){
                        if(isNaN(parseFloat(value)))
                            return"";
                        if(value!=null&&value!=undefined&&value!=-1)
                ...
                {field:'HOME_NUM_RATE',title:'电视<br/>渗透率',align:'center',halign:'center',width:sub_width*0.14,
                    formatter:function(value,rowData){
                        if(isNaN(parseFloat(value)))
                            return"";
                        if(value!=null&&value!=undefined&&value!=-1)
