...
function setDataToEchartMap6(params,url4realTimeSubList,url4mapToWhere){
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(d.length==0)
                ...
                for (var i = 0; i < d.length; i++) {
                    ...
                    if(stl=="--")
                        stl = "0";*/
                    obj.value = parseFloat(d[i].USE_RATE1);

                    vals.push(obj.value);

                    datas.push(obj);
                    datas2[org_name] = [d[i].USE_RATE,d[i].XY_RATE,d[i].ACTIVE_RATE];
                    //[d[i].USE_RATE,d[i].XY_RATE,d[i].ACTIVE_RATE];
                }
        ...
        complete:function(){
            echartMapReset6(min,max,params,color,datas,datas2);
            chart_map.hideLoading();
        }
    });
}
//地市 区域块
function echartMapReset6(min_num,max_num,params,color,data,data2){
...
//区县 柱状图
function setDataToEchartMap_bar_bureau6(params,url4realTimeSubList,url4mapToWhere){
    //chart.showLoading();
    //wait();
    var fullCityName = params.city_full_name;
    if(params.flag == 3){
    ...
    $.ajax({
        ...
        complete:function(){
            echartMapReset_bar_bureau6(min,max,params,color,datas,data_map);
            chart_map.hideLoading();
        }
    });
}
//区县 柱状图
function echartMapReset_bar_bureau6(min_num,max_num,params,color,data,data_map){
    var city_name = params.parent_name;
    if(city_name.indexOf("省")>-1)
        city_name = city_name.replace(/省/gi,'');
    try{
            ...
            series: [
                {
                    ...
                    geoIndex:0,
                    data: convertData_bureau(params.region_id,data,[98,97,96,95,94,0]),
                    symbolSize: [8,15],
                    label: {
                        normal: {
                            formatter: function(params){
