...
<head>
    ...
    function tab0_index(){
        $.post(url_summary_vc_sub,{"eaction":"get_info_all","substation": substation},function(data){
        ...
        //行政村数
        $.post(url_summary_vc_sub,{"eaction":"getVillageCellCnt","substation":substation},function(data){
    ...
    //加载 统计—市场占有率柱形图
	function tab0_bar(){
        $.post(url_summary_inside_vc_sub,{"eaction":"summary_bar","substation":substation},function(data){
            var month=[];
            var rate=[];
            var data = $.parseJSON(data);
    ...
    function load_data_list1(page,flag){
        var $list = $("#tab1_tbody");
        $.post(url_summary_inside_vc_sub,{"eaction":"market_grid","substation":substation,"page": page},function(data){
    ...
    function load_data_list2(page,flag){
        var $list = $("#tab2_tbody");
        $.post(url_summary_inside_vc_sub,{"eaction":"market_vc","substation":substation,"page":page,"tab2_text":tab2_text},function(data){
            var objs = $.parseJSON(data);
            if(page==0){
                if(objs.length){
            ...
            for(var i = 0,l = objs.length;i<l;i++){
                ...
                var row = "<tr>";
                row += "<td>"+ (++seq_num2) +"</td>";
                row += "<td><a href=\"javascript:void(0);\" onclick=\"toMap_grid('"+ d.VILLAGE_ID+"')\">"+ d.VILLAGE_NAME +"</a></td>";
    ...
    function load_data_list3(page,flag){
        var $list = $("#tab3_tbody");
        $.post(url_summary_inside_vc_sub,{"eaction":"market_user","substation":substation,"page":page,"tab3_text":tab3_text},function(data){
            var objs = $.parseJSON(data);
            if(page==0){
                if(objs.length){
    ...
    function toMap_grid(v_id){
        $.post(url_summary_inside_vc_sub,{"eaction":"getInsideOrgsByVcid","v_id":v_id},function(data){
            ...
            parent.clickToGridFromSub(data.UNION_ORG_CODE,data.BRANCH_NAME,data.ZOOM,data.GRID_NAME,data.STATION_ID);
            parent.global_func_operateVillage(v_id);
        });
    }
  </script>
</head>
<body>
    ...
    <div class="tab_head" id="span_switch">
      <span>统计</span> <span>网格</span> <span>行政村</span> <span>用户</span>
    </div>
