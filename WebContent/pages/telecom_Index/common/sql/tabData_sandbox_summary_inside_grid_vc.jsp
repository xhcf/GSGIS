...
<e:description>网格经理部分 begin</e:description>
  <e:description>市场部分begin</e:description>
    ...
    <e:description>市场 网格 网格经理无网格层 不使用
    ...
    <e:description>资源类 覆盖，实占</e:description>
    ...
    <e:case value="resource_obd">
        <e:q4l var="dataList">
            ...
            TT.RATE,
            COUNT(1) OVER() c_num,
            row_number() over(ORDER BY tt.BRIGADE_NAME) ROW_NUM
            FROM ${gis_user}.TB_GIS_COUNTRY_ODB_D TT, EDW.VW_TB_CDE_VILLAGE@GSEDW B
            WHERE TT.VILLAGE_ID = B.VILLAGE_ID
            AND B.GRID_ID = '${param.grid_id_short}'
            AND tt.ACCT_DAY =
            (${sql_part_acct_day1})
            <e:if condition="${!empty param.tab2_select}">
                and b.village_id = '${param.tab2_select}'
            </e:if>
            <e:if condition="${!empty param.tab2_text}">
                and (tt.eqp_no like '%${param.tab2_text}%' or tt.address like '%${param.tab2_text}%')
            </e:if>
            ORDER BY b.village_name,b.BRIGADE_NAME,eqp_no)
            WHERE ROW_NUM BETWEEN ${param.page} * 20 + 1 AND (${param.page} + 1) * 20
        </e:q4l>${e:java2json(dataList.list)}
    </e:case>
    ...
    <e:case value="stock_grid">
        <e:q4l var="dataList">
            ...
            WHERE A.GRID_ID = '${param.grid_id_short}'
            AND A.GRID_STATUS = '1'
